...
def main(nelx,nely,volfrac,penal,rmin,ft):
	...
	while change>0.01 and loop<2000:
		...
		dx=x-xold
#	op = op*0.9 + np.outer(dc,dc)*0.1
		op = op*0.9 + np.power(dc,2.)*0.1
		xp = xp*0.9  + np.power(dx,2.)*0.1


		# Optimality criteria
		xold[:]=x
		(x[:],g)=oc(nelx,nely,x,volfrac,dc,dv,g,op,xp)
...
def oc(nelx,nely,x,volfrac,dc,dv,g,op,xp):
	l1=0
	l2=1e9
	move=0.2
	...
	while (l2-l1)/(l1+l2)>1e-3:
		...
		tmp=np.minimum(np.maximum((dc + lmid*dv)/c_k,-move),move)
		xnew[:] = np.minimum(np.maximum(1e-6,x - tmp),1.)
