...
def main(nelx,nely,volfrac,pen,qen,muc,rmin,ft):
	...
	while change>0.01 and loop<2000:
		...
		dc[:] += 2. * gv * u[edofMat[:,1],0] * 1./4. * dxPena(muc,qen,xPhys)#qen * xPhys**(qen-1.)
		dc[:] += 2. * gv * u[edofMat[:,3],0] * 1./4. * dxPena(muc,qen,xPhys)#qen * xPhys**(qen-1.)
		dc[:] += 2. * gv * u[edofMat[:,5],0] * 1./4. * dxPena(muc,qen,xPhys)#qen * xPhys**(qen-1.)
		dc[:] += 2. * gv * u[edofMat[:,7],0] * 1./4. * dxPena(muc,qen,xPhys)#qen * xPhys**(qen-1.)
