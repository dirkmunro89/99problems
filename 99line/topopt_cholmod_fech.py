...
def main(nelx,nely,volfrac,penal,rmin,ft):
	...
	fixed=np.union1d(dofs[0:4:1], dofs[2*(nely+1)-1:2*(nely+1)-5:-1])
	din=nely+2
	...
	while change>0.01 and loop<2000:
		...
		obj=u[dout,0]#( (Emin+xPhys**penal*(Emax-Emin))*ce ).sum()
...
def oc(nelx,nely,x,volfrac,dc,dv,g):
	...
	while (l2-l1)/(l1+l2)>1e-6:
		lmid=0.5*(l2+l1)
		xnew[:]= np.maximum(0.0,np.maximum(x-move,np.minimum(1.0,np.minimum(x+move,x*(np.maximum(-dc,1e-9)/dv/lmid)**(1./3.)))))
#	xnew[:]= np.maximum(0.0,np.maximum(x-move,np.minimum(1.0,np.minimum(x+move,x*(np.maximum(np.absolute(-dc),1e-6)/dv/lmid)**(1./3.)))))
