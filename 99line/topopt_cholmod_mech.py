...
def main(nelx,nely,volfrac,penal,rmin,ft):
	...
	fixed=np.union1d(dofs[1:2*(nely+1)*(nelx+1)-1:2*(nely+1)],dofs[2*(nely+1)-1:2*(nely+1)-5:-1])
	din=1-1
	dout=2*nelx*(nely+1)+1-1
...
def oc(nelx,nely,x,volfrac,dc,dv,g):
	...
	while (l2-l1)/(l1+l2)>1e-4:
		lmid=0.5*(l2+l1)
		xnew[:]= np.maximum(0.0,np.maximum(x-move,np.minimum(1.0,np.minimum(x+move,x*(np.maximum(-dc,1e-10)/dv/lmid)**(1./3.)))))
