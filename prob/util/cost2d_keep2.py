...
#
    nng=np.dot(nab.T,b_k)/bag#/mag/bag ####!!!!
...
#
    ...
    [C, dC] = cost(nng)
    f = np.sum(C*x_f**(cen))
    dCdx = np.sum(dC*x_f**(cen)*(srf_gdx/bag-b_k[0]*np.dot(nab.T,b_k)/np.power(bag,3.)))
    dCdy = np.sum(dC*x_f**(cen)*(srf_gdy/bag-b_k[1]*np.dot(nab.T,b_k)/np.power(bag,3.)))
#   dCdx = np.sum(dC*(  mag*dnngdx     ))
#   dCdy = np.sum(dC*mag*dnngdy)
#
...
#
    s = np.sum(mag)
#   tmp = H # filter matrix (not transposed)
...
#   t = np.sum(C*mag)
#
#   rhs=dC*(b_k[0]/mag - np.dot(nab.T,b_k)*nab[0]/np.power(mag,3))*mag
    rhs=dC*(b_k[0]/bag)*x_f**cen# - np.dot(nab.T,b_k)*nab[0]/np.power(mag,2))/bag
    dcosdnx = np.asarray(rhs*Gx*tmp)[0,:]
#   rhs=dC*(b_k[1]/mag - np.dot(nab.T,b_k)*nab[1]/np.power(mag,3))*mag
    rhs=dC*(b_k[1]/bag)*x_f**cen#mag#- np.dot(nab.T,b_k)*nab[1]/np.power(mag,2))/bag
    dcosdny = np.asarray(rhs*Gy*tmp)[0,:]
    rhs=C*srf_gdx[np.newaxis]/mag
    dmagdnp1 = np.asarray(rhs*Gx*tmp)[0,:]
    rhs=C*srf_gdy[np.newaxis]/mag
    dmagdnp2 = np.asarray(rhs*Gy*tmp)[0,:]
    
    rhs=C*cen*x_f**(cen-1.)#*srf_gdy[np.newaxis]/mag
    dmagdex = np.asarray(rhs*tmp)[0,:]
#
    dfdx = dcosdnx+dcosdny+dmagdex#+dmagdnp1+dmagdnp2
#
#   measure of interior
#
