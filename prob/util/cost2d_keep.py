...
#
    [C, dC] = cost(nng)
    f = np.sum(C*mag)
    dCdx = np.sum(dC*(srf_gdx/bag-b_k[0]*np.dot(nab.T,b_k)/np.power(bag,3.)))
    dCdy = np.sum(dC*(srf_gdy/bag-b_k[1]*np.dot(nab.T,b_k)/np.power(bag,3.)))
...
#   rhs=dC*(b_k[0]/mag - np.dot(nab.T,b_k)*nab[0]/np.power(mag,3))*mag
    rhs=dC*(b_k[0] - np.dot(nab.T,b_k)*nab[0]/np.power(mag,2))/bag
    dcosdnx = np.asarray(rhs*Gx*tmp)[0,:]
#   rhs=dC*(b_k[1]/mag - np.dot(nab.T,b_k)*nab[1]/np.power(mag,3))*mag
    rhs=dC*(b_k[1] - np.dot(nab.T,b_k)*nab[1]/np.power(mag,2))/bag
    ...
    dmagdnp2 = np.asarray(rhs*Gy*tmp)[0,:]
#
    dfdx = dcosdnx+dcosdny+dmagdnp1+dmagdnp2
#
#   measure of interior
#
